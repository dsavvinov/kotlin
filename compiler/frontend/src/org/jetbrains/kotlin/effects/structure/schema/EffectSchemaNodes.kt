...
interface Effect : EsNode, Term {
    ...
    override fun castToSchema(): EffectSchema = EffectSchema(listOf(Imply(true.lift(), Cons(this, Nil))))
}
...
/**
 * Classes for representing consequent collection of nodes in form of tree.
 */
interface NodeSequence : EsNode, Term {
    override fun <T> accept(visitor: SchemaVisitor<T>): T = visitor.visit(this)
    override fun castToSchema(): EffectSchema = EffectSchema(listOf(Imply(true.lift(), this)))
}

object Nil : NodeSequence {
    override fun <T> accept(visitor: SchemaVisitor<T>): T = visitor.visit(this)
}

class Cons(val head: EsNode, val tail: NodeSequence) : NodeSequence {
    override fun <T> accept(visitor: SchemaVisitor<T>): T = visitor.visit(this)
}