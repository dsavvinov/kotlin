 ...
 */
data class Imply(override val left: EsNode, override val right: NodeSequence) : BinaryOperator {
    ...
    constructor(left: EsNode, effects: List<Effect>, outcome: Outcome?)
            : this(left, (effects + if (outcome != null) listOf(outcome) else listOf()).toNodeSequence())

    // TODO: unsafe cast here!
    override fun newInstance(left: EsNode, right: EsNode): BinaryOperator = Imply(left, right as NodeSequence)
    ...
    private fun (EffectSchema).flattenImply(right: EffectSchema): EffectSchema {
        ...
        for (leftClause in clauses) {
            ...
            val cond = EsEqual(leftOutcome.value, true.lift())
