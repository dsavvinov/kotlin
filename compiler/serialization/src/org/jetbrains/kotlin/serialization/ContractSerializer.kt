...
class ContractSerializer(private val descriptorSerializer: DescriptorSerializer) : SerializerExtension() {
    ...
    fun serializeContractOfFunctionIfAny(descriptor: FunctionDescriptor, proto: ProtoBuf.Function.Builder) {
        ...
        if (contractDescriptor != null) {
            proto.setContract(contractProto(contractDescriptor))
        }
    }
    ...
    private fun contractExpressionProto(contractDescriptionElement: ContractDescriptionElement, contractDescription: ContractDescription): ProtoBuf.Expression.Builder {
        return contractDescriptionElement.accept(object: ContractDescriptionVisitor<ProtoBuf.Expression.Builder, Unit> {
            ...
            override fun visitIsInstancePredicate(isInstancePredicate: IsInstancePredicate, data: Unit): ProtoBuf.Expression.Builder {
                ...
                // write rhs type
                builder.setIsInstanceType(descriptorSerializer.type(isInstancePredicate.type))
                builder.isInstanceTypeId = descriptorSerializer.typeId(isInstancePredicate.type)
